...
const SalesTable = ({ selectedDate, selectedYear, selectedMonth, weekDays = [], dashboardData = null, refreshDashboardData = null, dashboardLoading = false }) => {
  ...
  const getSalesChannelsConfig = () => {
    if (!completeOnboardingData || !completeOnboardingData['Sales Channels'] || !completeOnboardingData['Sales Channels'].data) {
      return {
        in_store: true, // Default to true for backward compatibility
        online: false,
        from_app: false
      };
    }
    ...
    return {
      in_store: salesChannelsData.in_store !== undefined ? salesChannelsData.in_store : true,
      online: salesChannelsData.online || false,
      from_app: salesChannelsData.from_app || false
    };
  };
  ...
  const areAllValuesZero = (weeklyData) => {
    ...
    return weeklyData.every(week => {
      ...
      return week.dailyData.every(day => {
        ...
        const actualSalesOnline = parseFloat(day.actualSalesOnline) || 0;
  ...
  const processDashboardData = () => {
    ...
    if (dashboardData['Sales Performance']) {
      ...
      const goals = {
        ...
        actualSalesOnline: parseFloat(salesPerformance.actual_sales_online) || 0,
    ...
    const allDailyEntries = dashboardData.daily_entries?.map((entry) => {
      const dailyData = {
        ...
        actualSalesOnline: entry['Sales Performance']?.actual_sales_online || 0,
    ...
    if (dailyData.length > 0) {
      const calculatedTotals = dailyData.reduce((acc, record) => {
        ...
        acc.actualSalesOnline += parseFloat(record.actualSalesOnline) || 0;
  ...
  const handleWeeklySubmit = async (weekData) => {
    try {
      ...
      const finalTotals = {
        ...
        actualSalesOnline: weeklyTotals.actualSalesOnline,
        netSalesActual: weeklyTotals.netSalesActual || (weeklyTotals.actualSalesInStore + weeklyTotals.actualSalesAppOnline + weeklyTotals.actualSalesOnline),
          ...
          <div className="overflow-x-auto">
            <Table
              ...
              summary={(pageData) => {
                ...
                // Calculate net sales actual total (including dynamic providers) - only include enabled sales channels
                const allSalesFields = [];
                if (salesChannelsConfig.in_store) {
                  allSalesFields.push('actualSalesInStore');
                }
                if (salesChannelsConfig.from_app) {
                  allSalesFields.push('actualSalesAppOnline');
                }
                ...
                // Calculate dynamic index for summary cells
                let cellIndex = 3; // Start after Day (0), Date (1), and Budgeted Sales (2)
              ...
              columns={[
                ...
                })),
                {
                  ...
                  render: (value, record) => {
                    ...
                    if (salesChannelsConfig.in_store) {
                      calculatedNetSales += parseFloat(record.actualSalesInStore) || 0;
                    }
                    if (salesChannelsConfig.from_app) {
                      calculatedNetSales += parseFloat(record.actualSalesAppOnline) || 0;
                    }
                    if (salesChannelsConfig.online) {
                      calculatedNetSales += parseFloat(record.actualSalesOnline) || 0;
                    }
                    ...
                    calculatedNetSales += providerSales;
                      ...
                      <div className="overflow-x-auto">
                        <Table
                          ...
                          summary={(pageData) => {
                            ...
                            // Calculate dynamic index for summary cells
                            let cellIndex = 2; // Start after Day (0) and Budgeted Sales (1)
