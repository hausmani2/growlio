...
const ReportCardPage = () => {
  ...
  const fetchSummaryData = async (startDate, endDate) => {
    try {
      ...
      if (!hasRestaurant || !restaurantId) {
        console.warn('⚠️ [ReportCardTestPage] Cannot fetch sales summary: No restaurant found');
        return;
      }
  ...
  const goals = {
    ...
    rent: apiSummary?.rent?.goal || 10,
    operatingExpenses: apiSummary?.expenses?.goal || 15,
  };
